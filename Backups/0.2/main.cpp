...
public:
    ...
    void onRender() override {
        ...
        // Render the STL Model with Test.bmp
        if (_stlModel && _testTexture) {
            _testTexture->bind();
            Renderer::drawMesh(*_stlModel, { 5, 0, -15 }, { 0, 0, 0 }, { 1, 1, 1 });
            _testTexture->unbind();
        }

        // Draw the Test Quad with Test.bmp
        if (_testTexture) {
            _testTexture->bind();
            glColor3f(1.0f, 1.0f, 1.0f); // Reset color for texture mapping
            glBegin(GL_QUADS);
            glTexCoord2f(0, 0); glVertex3f(-2, 2, -10);
            glTexCoord2f(1, 0); glVertex3f( 2, 2, -10);
            glTexCoord2f(1, 1); glVertex3f( 2, 6, -10);
            glTexCoord2f(0, 1); glVertex3f(-2, 6, -10);
            glEnd();
            _testTexture->unbind();
        }
